...
const boardMaker1 = (boardWithPins) => {
  ...
  let domString = `
    <div class="col-3">
      <div class="card  mb-3 myco-card border-0 rounded-2 primary" style="width: 18rem;" data-boards-boardTitle=${boardWithPins.boardTitle} id=${boardWithPins.id}>        
...
const boardMaker = (boardWithPins) => {
  ...
  for (let i = 0; i < boardWithPins.length; i += 1) {
    if (i === 0) {
      domString += `<img src="${boardWithPins[0].imgURL}" class="card-img-top1 ml-1 mt-1" alt="${boardWithPins.boardTitle}">`;
    }
    if (i === 1) {
      domString += `<img src="${boardWithPins[1].imgURL}" class="card-img-top1 ml-1 mt-1" alt="${boardWithPins.boardTitle}">`;
    }
    if (i === 2) {
      domString += `<img src="${boardWithPins[2].imgURL}" class="card-img-top1 ml-1 mt-1" alt="${boardWithPins.boardTitle}">`;
    }
  }
  domString += '</div>';
  ...
  for (let i = 3; i < boardWithPins.length; i += 1) {
    if (i === 3) {
      domString += `<img src="${boardWithPins[3].imgURL}" class="card-img-top1 ml-1 mt-1" alt="${boardWithPins.boardTitle}">`;
    }
    if (i === 4) {
      domString += `<img src="${boardWithPins[4].imgURL}" class="card-img-top1 ml-1 mt-1" alt="${boardWithPins.boardTitle}">`;
    }
    if (i === 5) {
      domString += `<img src="${boardWithPins[5].imgURL}" class="card-img-top1 ml-1 mt-1" alt="${boardWithPins.boardTitle}">`;
    }
  }
  domString += '</div>';
  domString += `
